...
void ls(char* path){
    ...
    //找到对应inode
    if(len>1)
        inodeNum = findInode(path);
    //读该inode指向的block
    for(int i=0;i<inodeTable[inodeNum].size;i++){
        memset(dir_items,0,sizeof(dir_items));
        readBlock(inodeTable[inodeNum].block_point[i],(char*)dir_items);
        //读该block的8个dir_item
        for(int j=0;j<8;j++){
            if(dir_items[j].valid==0)
                continue;
            if(dir_items[j].type==TYPE_FILE)
                printf("FILE:   %s\n",dir_items[j].name);
            if(dir_items[j].type==TYPE_DIR)
                printf("DIR:    %s\n",dir_items[j].name);
        }
    }
}
...
/*
    复制文件
    输入格式:
            cp /a /b    把根目录下的a文件复制给b文件
            cp /a/b /c  把根目录下a目录的b文件复制给c文件
*/
void cp(char* source,char* destination){
    //printf("源：%s  目的：%s\n",source,destination);
    inode inodeTable[1024];
    readInodeTable(inodeTable);
    int desInode = touch(destination);
    int srcInode = findInode(source);
    char buf[EXT2_BLOCK_SIZE];
    for(int i=0;i<inodeTable[srcInode].size;i++){
        readBlock(inodeTable[srcInode].block_point[i],buf);
        int freeBlock = findFreeBlock();
        inodeTable[desInode].block_point[i]=freeBlock;
        writeBlock(freeBlock,buf);
    }
}
